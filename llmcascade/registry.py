...
def resolve_auth_env(
    ...
    *,
    ...
    if provider:
        try:
            from llmcascade.provider_store import env_disabled, get_decrypted_key
...
def key_source(auth_env_var: str, *, provider: str, key_tier: KeyTier = "free") -> KeySource:
    try:
        from llmcascade.provider_store import env_disabled, get_decrypted_keys
...
def _read_custom_models() -> list[ModelConfig]:
    try:
        from llmcascade.model_store import load_custom_models
    except Exception:  # noqa: BLE001
        return []
    out: list[ModelConfig] = []
